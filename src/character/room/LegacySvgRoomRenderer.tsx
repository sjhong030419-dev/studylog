...
interface LegacySvgRoomRendererProps {
...
/**
 * The existing procedural SVG study room (docs/character-system.md §6),
 * extracted verbatim out of RoomScene.tsx so RoomScene can dispatch between
 * this and the new PixelRoomRenderer (docs/StudyLog_Pixel_Room_Asset_Spec_v1.0.md)
 * without changing a single pixel of this component's own output. This
 * stays the only renderer actually shown to users until a theme's real PNG
 * layers exist (roomThemeSupport.ts) — do not delete it before then.
 *
 * The room grows with level instead of the character itself.
 * `unlockedAdditions` is cumulative, so furniture never disappears once
 * earned.
 *
 ...
 */
export function LegacySvgRoomRenderer({ state, gender = 'boy', appearance, level, animated, characterScale = 1 }: LegacySvgRoomRendererProps) {
