...
export function StudyTimer({ onOpenShop, onOpenCapture, onOpenQuests, compactHome = false }: StudyTimerProps) {
  ...
  return (
    <div className={compactHome ? 'grid h-full min-h-0 min-w-0 w-full max-w-full grid-rows-[minmax(0,1fr)_auto_auto] place-items-center gap-0 overflow-hidden' : 'flex w-full flex-col items-center gap-5'}>
      <CharacterRoomCard
      ...
      {!hasNoSubjects && (
        <div className={`${compactHome ? '-mt-px w-full rounded-b-[22px] border-[3px] border-t border-white bg-(--color-home-card) px-2 pb-2 pt-1.5 shadow-[0_6px_0_rgba(91,68,101,0.10),0_14px_28px_rgba(74,52,83,0.16)]' : '-mt-2 w-full rounded-[20px] border border-white/80 bg-white/55 px-3 py-3 shadow-[0_8px_24px_rgba(88,64,98,0.08)] backdrop-blur'}`}>
