...
- **파일명 규칙:** `(gender)_(action)_(frame).png` — 예: `boy_study_01.png`
...
## 10. 설계 제안 — 실제 아트 자산 교체 경계

**현재 상태 (다시 한번 명확히):** 지금 화면에 보이는 캐릭터는 전부 `src/character/fallback/ChibiFallbackArt.tsx`의 SVG 코드로 그려진 placeholder다. 이 문서의 §9 파일 스펙을 만족하는 실제 PNG/스프라이트 에셋은 아직 하나도 존재하지 않는다. 어떤 보고서에서도 이 SVG를 "최종 프로덕션 아트"로 표현해서는 안 된다.

**실제 아트 자산을 넣을 때 건드릴 곳은 정확히 한 곳이다.**

- 앱 전체에서 캐릭터를 그리는 진입점은 `src/character/components/CharacterView.tsx` 하나뿐이다. 타이머, 스터디룸, 마이페이지, 상점, 캡처 카드 모두 이 컴포넌트를 `state`/`gender`/`appearance`/`size` props로만 호출하며, 내부 렌더링 방식(SVG인지 PNG 스프라이트인지)을 전혀 알지 못한다.
- `src/character/engine/spriteAssetMap.ts`의 `SPRITE_ASSETS_AVAILABLE` 플래그가 유일한 스위치다. `public/sprites/`에 §9 규칙(`(gender)_(action)_(frame).png`)대로 실제 파일을 넣고 이 플래그를 `true`로 바꾼 뒤, `CharacterView`의 `renderFrame`에서 `ChibiFallbackArt` 대신 `resolveSpriteFramePath()`가 가리키는 `<img>`를 렌더링하도록 분기만 추가하면 된다. `resolveSpriteFramePath()` 자체는 이미 구현되어 있다.
- 이 교체 작업은 `Timer`, `characterStateMachine`, `profileStore`, `AvatarShop`, `RoomScene`, `LogCaptureCard` 어느 파일도 수정할 필요가 없다 — 전부 `CharacterView`/`RoomScene`을 통해서만 캐릭터를 소비하기 때문이다.

**코스튬 슬롯도 같은 이유로 확장 가능하다.**

- `src/character/catalog/types.ts`의 `CharacterSlot`은 19개 고정 슬롯(예: `top`, `onePiece`, `hairFront`, `headAccessory` 등)을 정의하고, 각 상점 아이템은 `character/catalog/items.ts`에서 이 슬롯 중 하나에 데이터로 매핑된다. 렌더러(`ChibiFallbackArt`)는 아이템 ID를 직접 분기하지 않고 슬롯의 `zIndex` 순서대로만 그린다.
- `top`(상의)과 `onePiece`(원피스)는 서로 다른 슬롯이라 동시 장착 시 상호 배제(`incompatibleItemIds`)로 처리되며, 새 의상 카테고리가 추가되어도 `CharacterSlot`에 슬롯 하나를 추가하는 것으로 끝난다 — 렌더러에 새 `if` 분기가 필요 없다.

**결론:** 실제 캐릭터 아트가 준비되면 `spriteAssetMap.ts`의 플래그 전환 + `CharacterView.tsx`의 렌더 분기 추가만으로 교체가 끝나야 하며, 이번 MVP 마무리 단계에서 이 경계가 실제로 깨지지 않았는지가 §12의 핵심 인수 조건이다.
...
현재 상점 구성이 이 최소 세트를 정확히 충족한다 — 이번 단계에서 새 카탈로그 항목을 추가하지 않았다(§12 "이번 단계에서 대규모 신규 카탈로그를 만들지 말 것" 준수).
